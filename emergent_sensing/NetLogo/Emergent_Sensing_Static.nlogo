...
globals [
  ...
  circle
  ring
  light_count ; initialize light-value accumulator
  patch-movement
]
...
to setup
  ...
  set light_count 0
    ask patches
  [
    set light 0 ; intializes light to zero
  ]
  ...
  ;; calls the light initialization function
  let-light
end

to let-light
   repeat 7500 ; diffuses light a specified number of times
  [
    diffuse light 0.1 ; patch sharing 10% of its luminosity with 8 neighbors
    ask patch -5 5 [set light 1] ;; intializing a few light idealized sources
