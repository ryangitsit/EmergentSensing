...
globals [
  ...
  circle
  ring
  light_count ;; to accumulating light value for analysis
  patch-movement ;; defining movement of light by incrementing light source coordinates
]
...
to go
  ...
  clear-patches ;; clearing patches at each tick so that no diffused light is left behind

  repeat 500 ; diffuses light a specified number of times ;; cana also try 7500
  [
    diffuse light 0.8 ; patch sharing 10% of its luminosity with 8 neighbors
    ;; intializing a few light idealized sources with an offset defined by patch-movement
    ask patch (10 - patch-movement) (20 - patch-movement) [set light 1]
    ask patch (-30 - patch-movement) -30 [set light 1]
    ask patch (-5 - patch-movement) (5 - patch-movement) [set light 1]
    ask patch (10 - patch-movement) (20 - patch-movement) [set light 1]
    ask patch (-30 - patch-movement) (-30 - patch-movement) [set light 1]
  ]

  ;; incrementing patch movement to create motion of the light sources
  set patch-movement (patch-movement + 0.2)


  ;; gives light a yellow color that is proportionately shaded by luminosity
  ask patches
  [
  ...
  repeat update-freq [
    ask fish [
      avoid-light ;; incorporating the "gradient vector", which influences fish's behaviour wrt light
      flock       ;; calls "social-vector", which invokes flocking behavior
...
;; the below function is called repeatedly called in the go function
;; it acts on every fish in the system at every tick
;; defines fish behaviour on the basis of local light value
to avoid-light ;; fish light avoidance
   fd (light * .5) ;; for the speed-model, a foward speed of half the local light value is ADDED
   ;turn-away (towards one-of patches with[ light > 0.25 ]) (light * 15) ;; defines the turn model
   ;; the turn-away model prompts the fish to turn away from light
   ;; angular change is a funciont of the light value
   if ticks > 99 ;; give fish 100 ticks to flock before recording starts
      [set light_count light_count + (.25 * (1 - light))]
         ;; simple record 1/4 light value to account for frequencty update
         ;; this is equivalent to taking the average light value per tick
         ;; accumulates these light values for every fish over the whole experiment
         ;; averaging over fish and time is done in python analysis file "performance.py"
